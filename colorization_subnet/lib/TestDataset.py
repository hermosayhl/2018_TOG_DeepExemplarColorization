...
class TestDataset(data.Dataset):
	def __init__(self, input_dir, refer_dir, combo_dir, transform=None):

		self.combo_dir = combo_dir

		image_pairs = [(os.path.join(input_dir, it), os.path.join(refer_dir, it)) for it in os.listdir(input_dir)]

		# image_pairs = parse_images(data_root)
		if len(image_pairs) == 0:
			raise (RuntimeError("Found 0 image pairs in dataroot"))
		else:
			print("{} images to be processed !".format(len(image_pairs)))
	...
	def __getitem__(self, index):
		...
		image_comb_name = "input_%s_refer_%s" % (os.path.split(image_names[0])[-1], os.path.split(image_names[1])[-1])
		image_comb_name = image_comb_name.replace(".png", "").replace(".jpg", "")
		combo_path = osp.join(self.combo_dir, "%s.combo" % image_comb_name)
		...
		return inputs, os.path.split(image_path)[-1]
