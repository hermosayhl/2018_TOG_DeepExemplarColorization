...
if __name__ == '__main__':
    ...
    parser.add_argument('--refer_dir', type=str, default='refer')
    parser.add_argument('--combo_dir', type=str, default='combo')
    ...
    if opt.short_size > 0:
        test_dataset = TestDataset(opt.input_dir, opt.refer_dir, opt.combo_dir,
    ...
    size_unit = 8
    for cnt, (data, image_id) in enumerate(data_loader, 1):

        print("{}/{}  ===>  processing  {}".format(cnt, len(data_loader), image_id[0]))
        ...
        Image.fromarray(warpba_color_im).save(os.path.join(opt.out_dir, image_id[0]))